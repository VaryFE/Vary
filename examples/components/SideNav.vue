        ...
        <template v-if="item.groups">
          <div class="nav-group" v-for="(group, key) in item.groups" :key="key">
            ...
            <ul class="pure-menu-list">
              <li
                class="nav-item"
                v-for="(navItem, key) in group.list.filter(x=> !x.disabled)"
                :key="key"
              >
                <router-link
                  active-class="active"
...
export default class SideNavPage extends Vue {
  ...
  get langConfig() {
    ...
    return compoLang.filter((config:any) => config.lang === this.lang)[0]["nav"];
  }
  ...
  handlePathChange() {
    ...
    this.$nextTick(() => {
      this.hideAllMenu();
      let activeAnchor:any = this.$el.querySelector("a.active");
