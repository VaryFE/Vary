...
<template>
  <div class="page-changelog">
    <div class="heading">
      <el-button class="fr">
        <a href="<%= 3>/releases" target="_blank">GitHub Releases</a>
      </el-button>
      <%= 1 >
    </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let fragments = `<li><h3><a href="<%= 3>/releases/tag/v${release}" target="_blank">${release}</a></h3>`;
      ...
      for (let len = changeLogNodes.length, i = 2; i < len; i++) {
        ...
        if (node.tagName !== 'H3') {
          ...
          fragments += `</li><li><h3><a href="<%= 3>/releases/tag/v${release}" target="_blank">${release}</a></h3>`;
        }
      }
      fragments = fragments.replace(/#(\d+)/g, '<a href="<%= 3>/issues/$1" target="_blank">#$1</a>');
